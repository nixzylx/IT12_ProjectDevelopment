...
if (!isset($_SESSION['employeeID'])) { http_response_code(403); exit('Access denied'); }

$sale_id = intval($_GET['id'] ?? 0);
if (!$sale_id) { echo '<p style="color:red">Invalid sale ID.</p>'; exit(); }
...
if (!$sale) { echo '<p style="color:red">Sale not found.</p>'; exit(); }
...
");
$items = [];
while ($items_res && $r = $items_res->fetch_assoc()) $items[] = $r;

// payments
$pay_res = $conn->query("SELECT * FROM payments WHERE sales_id = $sale_id ORDER BY payment_date DESC");
$payments = [];
while ($pay_res && $r = $pay_res->fetch_assoc()) $payments[] = $r;
...
<style>
    ...
    .detail-label { font-size:11px; color:#888; text-transform:uppercase; margin-bottom:3px; }
    .detail-value { font-size:14px; font-weight:500; }
    .items-view-table { width:100%; border-collapse:collapse; font-size:13px; }
    .items-view-table th { background:#f9fafb; padding:9px 12px; text-align:left; font-size:11px; text-transform:uppercase; color:#888; border-bottom:1px solid #e8e7e3; }
    .items-view-table td { padding:10px 12px; border-bottom:1px solid #f3f4f6; }
    .payment-row { display:flex; justify-content:space-between; padding:10px 0; border-bottom:1px solid #f3f4f6; font-size:13px; }
    ...
    .section-title { font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:.5px; color:#444; margin:20px 0 10px; }
    .summary-box { background:#f9fafb; border-radius:10px; padding:16px; font-size:13px; }
    .summary-row { display:flex; justify-content:space-between; padding:5px 0; border-bottom:1px solid #f0f0f0; }
    .summary-row:last-child { border:none; font-size:15px; font-weight:700; margin-top:4px; }
</style>
...
<div style="display:flex; gap:20px; margin-top:16px; align-items:flex-start; flex-wrap:wrap;">
        ...
        <?php else: ?>
            ...
            <div class="payment-row">
                <div>
                    ...
                    <?php endif; ?>
                    <div style="font-size:11px; color:#aaa;"><?= date('M d, Y h:i A', strtotime($p['payment_date'])) ?></div>
                </div>
    ...
    <div style="width:220px;">
        ...
        <div class="summary-box">
            <div class="summary-row"><span>Subtotal</span><span>₱<?= number_format($sale['total_amount'], 2) ?></span></div>
            <div class="summary-row"><span>Discount</span><span style="color:#dc2626;">-₱<?= number_format($sale['discount'], 2) ?></span></div>
            <div class="summary-row"><span>Total</span><span>₱<?= number_format($sale['final_amount'], 2) ?></span></div>
            <div class="summary-row"><span>Paid</span><span style="color:#16a34a;">₱<?= number_format($total_paid, 2) ?></span></div>
